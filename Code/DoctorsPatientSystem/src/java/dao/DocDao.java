...
public int insert(DocDto ob){
    ...
    public int insertpat(DocDto ob){
        try {
            con=Connect.MyConnect();
           // if(ob.getType().equals("Doctors")){
            ps = con.prepareStatement("insert into patientinfo(id,name,pswd,loginStatus,path,type,email,Address,Phone,Occupation,Institution,active)values(?,?,?,?,?,?,?,?,?,?,?,?)");
            ps.setString(1,ob.getId());
            ps.setString(2,ob.getName());
            ps.setString(3,ob.getPassword());
            ps.setInt(4,0);
