...
public class RegAction extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ob.setType(request.getParameter("select"));
        sel=request.getParameter("select");
        out.write("type"+sel);
        DocDao ob1 = new DocDao();
        if (request.getParameter("select").equals("Doctors")) {
            if (ob1.insert(ob) == 1) {
                response.sendRedirect("registration.jsp?msg=Registered Suceessfully");

            } else {
                response.sendRedirect("registration.jsp?msg=Registration Failed");
            }
        } else {
            if (ob1.insertpat(ob) == 1) {
                response.sendRedirect("registration.jsp?msg=Pat Registered Suceessfully");
